...
/**
 * Hyphenates a word using the German maximal-onset rule: a single intervocalic
 * consonant joins the following syllable; inside a cluster the last indivisible
 * unit joins the following syllable and the rest close the previous one.
 */
export function syllabify(raw: string): string[] {
  ...
  for (let n = 0; n < nuclei.length - 1; n += 1) {
    ...
    let cut = clusterEnd - 1;
    for (const digraph of CONSONANT_DIGRAPHS) {
      if (cluster.endsWith(digraph) && cluster.length > digraph.length) {
        cut = clusterEnd - digraph.length;
        break;
      }
    }
